...
We only start counting a consecutive sequence when `num - 1` is not in the set, which meanst the current number is the start of the sequenc. From there we would count forward as long as the next number exists.

Since each number is only counted one time in a sequence, the total runtime would be O(n) (I;m pretty sure). Without the check shown above it would be of O(n^2) complexity, as it would have to repeatedly count the same sequences starting from every number. The order of the list doesn't matter because we only check if numbers exist in the list not by order.
