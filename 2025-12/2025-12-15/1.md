...
2. `enumerate` python function essentially adds a counter to an iterable and returns it as a enumerate (one by one) object, in a tuple of `(index, value)`. This simplifies loops by providing both the position and item simultaneously. The rest of the code works by using hash maps, essentially it will store a previous seen value in a dictionary and enumerate through the list, if the `target - value` is inside of a previous seen value then its returns. This is a one pass solution it will never use the same element twice but will still check every element inside of the array.
